...
class MessageDetection(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Werte aus values.json laden (zur Item-Erkennung)
        self.values_data = load_json(VALUES_FILE)
        # Alle Item-Namen sammeln (über alle Kategorien)
        self.all_items = []
        ...
        # Alias-Liste (hard-coded) für alternative Itemnamen
        self.alias_mapping = {
            # items:
    ...
    async def on_message(self, message: discord.Message):
        # Ignoriere Bots und globalen Cooldown
        ...
        if key in self.pending_serial:
            ...
            try:
                ...
                await self.safe_send(message.channel, f"Error: {e} \n-# The serial number you gave is most likely either too high or too low.")
                return

            item_data = self.get_item_data(pending["item"])
            response = (
                f"__**{pending['item']}**__\n"
                f"- Value (Serial #{serial}): {self.format_cash(value)}\n"
