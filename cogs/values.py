...
class Values(commands.Cog):
    ...
    def parse_price_string(self, price_str: str) -> int:
        """
        Converts a price string (e.g. '8.2M-8.3M' or '70k') into an integer.
        If the string is a range, the average is returned.
        """
        if "-" in price_str:
    ...
    def format_cash(self, amount: int) -> str:
        """
        Formats an integer amount into a string with 'k' or 'M' suffix.
        """
        if amount >= 1000000:
    ...
    def get_item_data(self, item_name: str):
        """Sucht in values.json nach dem Item (über alle Gruppen)."""
    ...
    def get_item_category(self, item_name: str) -> Optional[str]:
        """Ermittelt, zu welcher Kategorie (Gruppe) ein Item gehört."""
        for group in ["items", "event_items", "miscellaneous_items", "kukri_items"]:
            if group in self.item_data and item_name in self.item_data[group]:
                return group
        return None
    ...
    async def value_command(self, ctx, *args):
        if len(args) < 1:
            ...
            return
        # Falls mehr als ein Argument vorliegt und das letzte numerisch ist, nutze es als Serial
    ...
    async def value_slash_command(self, interaction: discord.Interaction, item: str, serial: int = None):
        # Zusätzliche Prüfung: Falls das Item aus "event_items" oder "miscellaneous_items" stammt,
        # wird geprüft, ob eine Seriennummer übergeben wurde. Falls ja, wird eine ephemeral Nachricht gesendet.
        ...
        if category in ["event_items", "miscellaneous_items"]:
            if serial is not None:
                ...
                return
            # Für diese Items wird die Seriennummer ignoriert
            serial = None
        ...
        # Für Items aus event/misc wird keine Serieninformation im Embed angezeigt
        if category in ["event_items", "miscellaneous_items"]:
            title = f"{item}"
        else:
            title = f"{item} - {serial if serial is not None else 'High serial'}"
